...
public class WeatherStationModelTinkerforge extends WeatherStationModel {
    ...
    private void addAmbientLight(String uid) {
        ...
        ambient.addIlluminanceListener(illuminance -> {
            final BigDecimal pressure = decimal((int)illuminance).divide(decimal(10));
        ...
        tilt.addTiltStateListener(tiltState -> {
            ...
            switch (tiltState) {
                case BrickletTilt.TILT_STATE_CLOSED:
                    state = State.TILT_STATE_CLOSED;
                    break;
                case BrickletTilt.TILT_STATE_OPEN:
                    state = State.TILT_STATE_OPEN;
                    break;
                case BrickletTilt.TILT_STATE_CLOSED_VIBRATING:
                    state = State.TILT_STATE_CLOSED_VIBRATING;
                    break;
            }
